# Keep Codex Remote Control and Official Plugins While Using Third-Party APIs: CCodex Switch Setup Guide

> Applies to CCodex Switch v3.16.1 and later. This guide is based on the current code, user manual, and v3.16.1 release notes. Screenshots use de-identified sample data and do not include real Access Tokens or API keys.
...
1. In the CCodex Switch Codex panel, switch to `OpenAI Official`.
2. Start Codex and log in once with an official ChatGPT / Codex account. A Free subscription is enough.
3. Return to CCodex Switch and enable `Settings -> General -> Codex App Enhancements -> Keep official login when switching third-party providers`.
...
- CCodex Switch v3.16.1 or later.
...
Open CCodex Switch and switch to the top-level `Codex` tab. First select the `OpenAI Official` provider, or add it from the preset providers if it is missing, and make it the current provider.
...
Return to CCodex Switch and open:
...
If it only supports OpenAI Chat Completions, which is common for DeepSeek / Kimi / MiniMax paths, local routing must be enabled so CCodex Switch can convert Codex Responses requests into Chat Completions requests.
...
After takeover, Codex's live `config.toml` temporarily points to the CCodex Switch local route. The real third-party API key remains in the CCodex Switch provider configuration, and is projected into the `experimental_bearer_token` in `config.toml` when providers are switched.
...
- In CCodex Switch, the current Codex provider is the third-party provider.
...
After `Keep official login when switching third-party providers` is enabled, CCodex Switch takes the third-party provider API key from the provider configuration and writes it under the current provider in `config.toml`:
...
If the provider uses the Chat Completions protocol, CCodex Switch local routing adds another conversion layer:

```text
Codex Responses request
        |
CCodex Switch local route
        |
...
That does not mean model requests are still going to official OpenAI. Actual traffic is determined by the current Codex provider in CCodex Switch, `config.toml`, and local routing logs.
...
Codex reads the model catalog at startup. Even if CCodex Switch has generated a new model catalog, a running Codex process may not hot-load it, so restart Codex after editing model mappings.
...
This is expected. Official account information comes from `auth.json`; the actual model provider comes from `config.toml` and the current provider in CCodex Switch.

**Is a Free subscription really enough?**

Yes. The official account is mainly used to obtain and preserve the official login state required by Codex App. Third-party model requests use the third-party API key configured in CCodex Switch.

**What should I do if official plugins or mobile remote control still do not work?**

Switch back to `OpenAI Official`, restart Codex, and complete official login once. Then confirm `Settings -> General -> Codex App Enhancements -> Keep official login when switching third-party providers` is enabled in CCodex Switch before switching back to the third-party provider.
...
Not recommended. CCodex Switch tries to prevent switching to official providers while local routing takeover is active, because accessing official APIs through a proxy may create account risk. Use official login only to preserve `auth.json`, and route model traffic to third-party providers.
...
- [CCodex Switch v3.16.1 Release Note](../release-notes/v3.16.1-en.md)
