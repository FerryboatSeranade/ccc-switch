...
Codex Switch v3.16.2 是 v3.16.1 之后的一版维护更新。在上一版集中处理 Codex 官方鉴权与本地路由接管的安全问题之后，这一版把重心放在两件事上：一是拓宽数据的可携带性和用量观测——新增 S3 兼容云同步（WebDAV 之外的第二套云备份后端）、OpenCode 会话用量同步，以及面向官方订阅的额度统计模板；二是继续打磨 Codex 通过 Chat Completions 路由第三方供应商时暴露出来的边角问题——流式截断判定、空 tools 下的 `tool_choice`、自定义工具元数据、推理 token 统计、文件 / 音频附件转换等。
...
本地代理现在会响应 Codex CLI 启动时探测的 `GET /v1/models`，返回 Codex Switch 托管的 Codex 模型目录。同时加入了过期目录守卫：解析 live 的 `config.toml`，仅当 `model_catalog_json` 仍指向 Codex Switch 持有的目录文件时才提供，避免把上一个供应商遗留的目录暴露给 Codex（#3818）。
...
当 Chat Completions 上游在没有 `finish_reason` 或 `[DONE]` 的情况下结束流时，Codex Switch 不再把它当作正常完成：只有流真正结束才正常收尾；已产出部分内容时发出 incomplete（`max_output_tokens`）响应；完全没有产出时发出失败的 `stream_truncated` 事件。晚到的推理内容也会回填到仍在进行的流式工具调用上。
...
当 Codex 上游网关以 HTTP 413 拒绝过大的请求体时，代理现在返回专门的提示，说明这是供应商服务端的请求体大小限制（而非 Codex Switch 本地限制），并给出可操作的恢复步骤（运行 `/compact`、移除大段日志或内联图片，或请供应商调高限制），不再原样回显上游的 HTML 错误页。
...
在 Windows 上退出 Codex Switch 可能会留下一个失效的托盘图标，直到鼠标划过才消失。现在应用会在退出前显式移除托盘图标，让它随进程结束干净消失（#3797）。

### Windows 任务栏图标

在运行时显式设置 Windows AppUserModelID，并给安装器生成的桌面和开始菜单快捷方式写入相同的 ID 和产品图标，让 Codex Switch 在任务栏上显示正确图标并正确归组（#3457）。
...
| 文件                                     | 说明                                |
| ---------------------------------------- | ----------------------------------- |
| `CodexSwitch-v3.16.2-Windows.msi`          | **推荐** - MSI 安装包，支持自动更新 |
| `CodexSwitch-v3.16.2-Windows-Portable.zip` | 便携版，解压即用，不写入注册表      |

### macOS

| 文件                             | 说明                                          |
| -------------------------------- | --------------------------------------------- |
| `CodexSwitch-v3.16.2-macOS.dmg`    | **推荐** - DMG 安装包，拖入 Applications 即可 |
| `CodexSwitch-v3.16.2-macOS.zip`    | 解压后拖入 Applications，Universal Binary     |
| `CodexSwitch-v3.16.2-macOS.tar.gz` | 用于 Homebrew 安装和自动更新                  |
