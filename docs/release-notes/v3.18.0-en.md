...
xAI's Grok CLI ("Grok Build", live config `~/.grok/config.toml`) is now a first-class citizen in Codex Switch: provider add / import / one-click switch (with a "restart Grok Build to apply" toast), app visibility and config-directory override settings, session manager and usage dashboard coverage, prompts with first-launch auto-import, `cccswitch://` deep-link provider import, and local proxy takeover — with its own `/grokbuild/v1/responses` route namespace, an independent failover queue, and per-app proxy settings; forwarding reuses Codex's Responses path but never shares Codex's provider namespace or circuit-breaker state.
...
Default Skill repositories were re-seeded on every startup by a "supplement missing defaults" pass, so a default repo you deleted silently returned on the next launch. Seeding is now one-time per database, tracked by a settings flag; databases that already contain repositories at upgrade time get the flag set without any re-seeding, so existing selections are untouched. ([#5356](https://github.com/FerryboatSeranade/ccc-switch/pull/5356))

### First-Run Tray Language Follows the System Locale

Before any language was chosen in settings, the tray menu was hardcoded to Simplified Chinese — on English / Japanese / Traditional-Chinese systems the main UI correctly followed the OS locale while the tray disagreed, until the user switched language once. The tray now derives its first-run language from the OS locale with the same precedence rules as the frontend (including `zh-TW` / `zh-HK` / `zh-Hant` → Traditional Chinese); an explicitly chosen language always wins, and unreadable locales fall back to Chinese as before. ([#4355](https://github.com/FerryboatSeranade/ccc-switch/pull/4355))
...
- [#5453](https://github.com/FerryboatSeranade/ccc-switch/pull/5453): first-class Grok Build support (the core of the eighth managed app), thanks @YUZHEthefool.
- [#5508](https://github.com/FerryboatSeranade/ccc-switch/pull/5508): attach reasoning forward across the Responses→Chat bridge, thanks @ka79376046.
- [#5310](https://github.com/FerryboatSeranade/ccc-switch/pull/5310): preserve streamed parallel tool-call identity and order, thanks @SaladDay.
- [#5315](https://github.com/FerryboatSeranade/ccc-switch/pull/5315): default Codex tool parameters to an object schema, thanks @Komikawayi.
- [#4706](https://github.com/FerryboatSeranade/ccc-switch/pull/4706): normalize tool parameter types for strict OpenAI-compatible providers, thanks @Ryan2128.
- [#5356](https://github.com/FerryboatSeranade/ccc-switch/pull/5356): keep deleted default Skill repositories deleted, thanks @allenxu09.
- [#4355](https://github.com/FerryboatSeranade/ccc-switch/pull/4355): first-run tray language follows the system locale, thanks @LaiYueTing.
- [#5138](https://github.com/FerryboatSeranade/ccc-switch/pull/5138): extend backend CI to Linux / Windows / macOS, thanks @zayokami.
...
| File                                     | Description                                      |
| ---------------------------------------- | ------------------------------------------------ |
| `CodexSwitch-v3.18.0-Windows.msi`          | **Recommended** - MSI installer with auto-update |
| `CodexSwitch-v3.18.0-Windows-Portable.zip` | Portable build, unzip and run                    |
...
| File                             | Description                                           |
| -------------------------------- | ----------------------------------------------------- |
| `CodexSwitch-v3.18.0-macOS.dmg`    | **Recommended** - DMG installer, drag to Applications |
| `CodexSwitch-v3.18.0-macOS.zip`    | Unzip and drag to Applications, Universal Binary      |
| `CodexSwitch-v3.18.0-macOS.tar.gz` | For Homebrew install and auto-update                  |
...
- `CodexSwitch-v3.18.0-Linux-x86_64.AppImage` / `.deb` / `.rpm`
- `CodexSwitch-v3.18.0-Linux-arm64.AppImage` / `.deb` / `.rpm`
