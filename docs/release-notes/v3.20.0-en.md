...
- **Manage Pi with Codex Switch**: the ninth managed app ([#6064](https://github.com/FerryboatSeranade/ccc-switch/pull/6064)). The provider form is a structured editor over Pi's native schema, with 58 built-in presets and a 57-model capability catalog; the prompt library, `SYSTEM.md` / `APPEND_SYSTEM.md` editors, slash-command templates, Skills, a session browser and install/upgrade all come along. Pi's own login, default provider and default model are **never touched**. First launch imports the providers already in `models.json` — see [Upgrade Notes](#pis-first-launch-imports-existing-providers-and-backfills-historical-usage).
- **See Pi's session usage in the dashboard**: per-model tokens, costs, errors and aborted turns, under its own "Pi (Session)" source and app filter; incremental resync parses only what was appended to each file ([#6463](https://github.com/FerryboatSeranade/ccc-switch/pull/6463)).
- **Pick an authorized ChatGPT account right when adding a Codex official provider**: sign in to any number of accounts in the Auth Center, then bind a new official card to one of them from a dropdown; or keep the classic route — an unbound official card that follows the Codex CLI's own login, reading the local access token. Switching to a bound card writes that account's complete token bundle — the bare `codex` CLI runs as that account too, and can self-refresh ([#3879](https://github.com/FerryboatSeranade/ccc-switch/pull/3879), [#6535](https://github.com/FerryboatSeranade/ccc-switch/pull/6535)).
- **Use Claude Code's built-in WebSearch under GPT routing**: searches execute upstream, results come back with citations, counts land in usage; the Alpha Search endpoint newer Codex clients call no longer 404s either ([#5681](https://github.com/FerryboatSeranade/ccc-switch/pull/5681)).
- **Escape the v3.19.2 WSL breakage**: v3.19.2 could not update or switch existing configurations on `\\wsl.localhost` paths ([#6188](https://github.com/FerryboatSeranade/ccc-switch/issues/6188)), with no workaround on that version — upgrade straight to this release, and CI now tests against real WSL2 filesystems from here on ([#6233](https://github.com/FerryboatSeranade/ccc-switch/pull/6233)).
- **Have Windows version detection see what your terminal sees**: detection now merges the registry PATH (user + machine), scans standalone installer directories, and probes the PATH default first — "not installed" after in-app self-update, winget-installed Claude Code going unseen, and a stale npm shim masking the newer version: a five-issue family fixed at once ([#6284](https://github.com/FerryboatSeranade/ccc-switch/pull/6284)).
- **Declare the reasoning levels each model really supports**: the Codex model catalog takes a per-model multi-select over the eight canonical levels plus a default, and the presets pre-fill the real tiers from vendor documentation ([#6228](https://github.com/FerryboatSeranade/ccc-switch/pull/6228)).
- **Trust your backups**: SQL backups round-trip every value and truncated files are rejected before import ([#6146](https://github.com/FerryboatSeranade/ccc-switch/pull/6146)); backup files publish atomically, restores validate in a staging database first, `.db` restores rebuild each app's live config, and every sync/restore path runs strictly one at a time ([#6147](https://github.com/FerryboatSeranade/ccc-switch/pull/6147)).
- **Search across hundreds of models**: every model dropdown in every app becomes a type-to-filter combobox, matching on model id and vendor name alike ([#6285](https://github.com/FerryboatSeranade/ccc-switch/pull/6285)).
- **Start without the white/black flash**: the theme applies synchronously before first paint, and on Windows the window stays hidden until the page has loaded ([#6252](https://github.com/FerryboatSeranade/ccc-switch/pull/6252)).
...
>
> Codex Switch is a **fully free and open-source** desktop app, and we **do not charge users any fees**. Please only obtain the software through the official channels listed below:
>
> | Channel            | Only Official                                                                  |
> | ------------------ | ------------------------------------------------------------------------------ |
> | Website            | **[GitHub Releases](https://github.com/FerryboatSeranade/ccc-switch/releases)**                                         |
> | Source             | **[github.com/FerryboatSeranade/ccc-switch](https://github.com/FerryboatSeranade/ccc-switch)** |
> | Downloads          | **[GitHub Releases](https://github.com/FerryboatSeranade/ccc-switch/releases)**        |
> | Author             | **[@farion1231](https://github.com/farion1231)**                               |
> | Report an Imposter | **[GitHub Issues](https://github.com/FerryboatSeranade/ccc-switch/issues)**            |
>
> **Any "Codex Switch" website or client that asks you for payment, top-ups, or login credentials is fake.** If you have been tricked into paying, stop the transaction immediately and file a report through GitHub Issues.
...
The account picker is a labelled dropdown with a direct jump into the Auth Center; a transient **failure to load** account status now shows an alert with Retry instead of reading as "not signed in", and no longer silently unbinds the account on the next save. Sign-out and account removal are serialized against provider switching; OAuth request timeouts tighten from ten minutes to 30 seconds; and signing out also cancels a device login whose network round-trip is still in flight — an abandoned flow can no longer complete in the background and quietly restore an account ([#6506](https://github.com/FerryboatSeranade/ccc-switch/pull/6506)). Requests through takeover are validated against the bound account: a Codex session still authenticated as a different ChatGPT account gets an explicit "restart Codex" error instead of silently billing the wrong account. ([#3879](https://github.com/FerryboatSeranade/ccc-switch/pull/3879), [#6535](https://github.com/FerryboatSeranade/ccc-switch/pull/6535), [#6506](https://github.com/FerryboatSeranade/ccc-switch/pull/6506))

### Codex Model Catalog: Per-Model Reasoning Levels

Each model row in the Codex provider form can now declare exactly which reasoning efforts its upstream accepts — a multi-select over the eight canonical levels (none through ultra) plus an optional default — and the generated catalog carries them into Codex's picker instead of inheriting the base template's. Unknown values are dropped (a typo never reaches Codex), an explicit default is validated against the declared set, and the override applies on both the native-Responses and the chat-conversion paths. The proxy transforms learned `ultra` at the same time: previously, picking the deepest level on a routed provider silently disabled extended thinking (Anthropic path) or dropped the effort parameter; it now maps to the upstream's deepest legal tier. ([#6228](https://github.com/FerryboatSeranade/ccc-switch/pull/6228), [#6181](https://github.com/FerryboatSeranade/ccc-switch/issues/6181))
...
- **New presets**: PPIO (vendor-contributed), JieKou AI (vendor-contributed) and XycAi (partner), each across the apps they support. ([#6239](https://github.com/FerryboatSeranade/ccc-switch/pull/6239), [#6356](https://github.com/FerryboatSeranade/ccc-switch/pull/6356))
...
- **OpenClaw "Set as default" asks which model and preserves your fallbacks**: setting a provider as default always picked its first model and replaced whatever fallback chain `openclaw.json` contained with a synthetic one. Multi-model providers now get a picker, and the write merges into the existing default-model block, preserving fallbacks and unknown keys. ([#6201](https://github.com/FerryboatSeranade/ccc-switch/pull/6201))
- **A user-owned Codex `model_catalog_json` is no longer overwritten**: switching providers unconditionally repointed `model_catalog_json` in `~/.codex/config.toml` at the app-generated catalog, discarding a custom catalog path. The pointer is now claimed only when absent or already Codex Switch's own filename. Prevention only — a pointer already rewritten by an earlier version is not restored, see Upgrade Notes. ([#6087](https://github.com/FerryboatSeranade/ccc-switch/pull/6087))
...
The schema migrates from v16 to v17 (the session-usage dedup ledger for the Pi importer), with a backup created automatically before migrating. After this version has run once, older Codex Switch builds refuse to open the database — downgrading requires restoring that backup.
...
**Third-party provider routing**: when the Codex Switch local proxy converts and forwards Codex, Claude Desktop, or Grok Build requests to a third-party provider, each provider has different constraints on billing, compliance, and data retention. Please read the target provider's terms of service before use.

By enabling these features, users accept the associated risks. Codex Switch is not responsible for any account restriction, warning, or service suspension resulting from their use.
...
- Thanks to @SaladDay — 19 commits, this release's largest contributor: the entire Pi line ([#6064](https://github.com/FerryboatSeranade/ccc-switch/pull/6064), [#6463](https://github.com/FerryboatSeranade/ccc-switch/pull/6463)), the entire Codex multi-account line ([#3879](https://github.com/FerryboatSeranade/ccc-switch/pull/3879), [#6535](https://github.com/FerryboatSeranade/ccc-switch/pull/6535), [#6506](https://github.com/FerryboatSeranade/ccc-switch/pull/6506), [#6537](https://github.com/FerryboatSeranade/ccc-switch/pull/6537)), the ten-PR form consistency pass, the Windows startup-flash fix ([#6252](https://github.com/FerryboatSeranade/ccc-switch/pull/6252)) and the Grok Build usage-field fix.
- Thanks to @YUZHEthefool: the two backup and sync hardening waves ([#6146](https://github.com/FerryboatSeranade/ccc-switch/pull/6146), [#6147](https://github.com/FerryboatSeranade/ccc-switch/pull/6147)) — the three big data-reliability entries in "Fixed" are this work; plus the Windows CLI detection overhaul ([#6284](https://github.com/FerryboatSeranade/ccc-switch/pull/6284)), model-picker fuzzy search ([#6285](https://github.com/FerryboatSeranade/ccc-switch/pull/6285), [#6353](https://github.com/FerryboatSeranade/ccc-switch/pull/6353)), the Zhipu CREDIT_LIMIT fix and the Grok Build form alignment.
- Thanks to @allenxu09: the hotfix for v3.19.2's WSL write regression ([#6232](https://github.com/FerryboatSeranade/ccc-switch/pull/6232)) and CI coverage on real WSL2 filesystems ([#6233](https://github.com/FerryboatSeranade/ccc-switch/pull/6233)) — fixing the bug and closing the test gap that let it ship.
- Thanks to @Program120: the WebSearch and Alpha Search proxy bridge ([#5681](https://github.com/FerryboatSeranade/ccc-switch/pull/5681)) — one person opened this release's third main line.
- Thanks to @stofancy: per-model reasoning levels in the Codex catalog ([#6228](https://github.com/FerryboatSeranade/ccc-switch/pull/6228)) — the foundation of this release's reasoning-tier pass.
- Thanks to @zayokami: OpenCode Zen reasoning-effort routing ([#6123](https://github.com/FerryboatSeranade/ccc-switch/pull/6123)), DeepSeek cache-hit accounting ([#6126](https://github.com/FerryboatSeranade/ccc-switch/pull/6126)) and the Grok Build wording fix ([#6511](https://github.com/FerryboatSeranade/ccc-switch/pull/6511)).
- Thanks to @yovinchen: the macOS input-method corruption fix and its hardening ([#6333](https://github.com/FerryboatSeranade/ccc-switch/pull/6333), [#6507](https://github.com/FerryboatSeranade/ccc-switch/pull/6507)).
- Thanks to @Hexc01: the multi-year usage-trend alignment fix ([#6337](https://github.com/FerryboatSeranade/ccc-switch/pull/6337)).
- Thanks to @misaka-myu: respecting a user-owned `model_catalog_json` ([#6087](https://github.com/FerryboatSeranade/ccc-switch/pull/6087)).
- Thanks also to @hu-miao and @jiekouai for contributing their platforms' presets ([#6239](https://github.com/FerryboatSeranade/ccc-switch/pull/6239), [#6356](https://github.com/FerryboatSeranade/ccc-switch/pull/6356)).

### Issue Reports

- Thanks to @vxzhong for the WSL write-regression report in [#6188](https://github.com/FerryboatSeranade/ccc-switch/issues/6188) — pinpointing os error 50 framed the hotfix directly.
- Thanks to the Windows CLI-detection five-issue family: @RonsonNamek ([#6061](https://github.com/FerryboatSeranade/ccc-switch/issues/6061)), @gaoqiong001 ([#6278](https://github.com/FerryboatSeranade/ccc-switch/issues/6278)), @lovelyhjqhs ([#6047](https://github.com/FerryboatSeranade/ccc-switch/issues/6047)), @baoyu0 ([#4366](https://github.com/FerryboatSeranade/ccc-switch/issues/4366)) — and @jiangliushi666, who flagged the PATH probe-order problem earlier in [#4701](https://github.com/FerryboatSeranade/ccc-switch/pull/4701).
- Thanks to @HyskoaMorroh for reporting the takeover restore wiping the ChatGPT login ([#6277](https://github.com/FerryboatSeranade/ccc-switch/issues/6277)), @CacinieP for the precise IME-corruption reproduction ([#6308](https://github.com/FerryboatSeranade/ccc-switch/issues/6308)), @FishV6A for the startup flash ([#6182](https://github.com/FerryboatSeranade/ccc-switch/issues/6182)), and @Destiny4073 for the MSI registry mis-write ([#6283](https://github.com/FerryboatSeranade/ccc-switch/issues/6283)).
- Thanks to @totoneei ([#6302](https://github.com/FerryboatSeranade/ccc-switch/issues/6302)), @zju-zhanglu ([#6153](https://github.com/FerryboatSeranade/ccc-switch/issues/6153)), @xiaoyu753 ([#6073](https://github.com/FerryboatSeranade/ccc-switch/issues/6073)), @GengchenXU ([#6140](https://github.com/FerryboatSeranade/ccc-switch/issues/6140)), @HOnnTaka ([#6112](https://github.com/FerryboatSeranade/ccc-switch/issues/6112)), @Xiao0219 ([#6171](https://github.com/FerryboatSeranade/ccc-switch/issues/6171)), @STEVENTAN100 ([#6181](https://github.com/FerryboatSeranade/ccc-switch/issues/6181)), @20130101 ([#6070](https://github.com/FerryboatSeranade/ccc-switch/issues/6070)) and @leoncdq ([#6448](https://github.com/FerryboatSeranade/ccc-switch/issues/6448)) — a whole batch of usage, quota and tier corrections started from these reports.

---

## Download & Install

Visit [Releases](https://github.com/FerryboatSeranade/ccc-switch/releases/latest) and download the build for your system, or get it from the official site [GitHub Releases](https://github.com/FerryboatSeranade/ccc-switch/releases) (downloads are distributed through Cloudflare edge nodes and do not depend on GitHub being reachable).
...
| File                                     | Description                                      |
| ---------------------------------------- | ------------------------------------------------ |
| `CodexSwitch-v3.20.0-Windows.msi`          | **Recommended** - MSI installer with auto-update |
| `CodexSwitch-v3.20.0-Windows-Portable.zip` | Portable build, unzip and run                    |
...
| File                             | Description                                           |
| -------------------------------- | ----------------------------------------------------- |
| `CodexSwitch-v3.20.0-macOS.dmg`    | **Recommended** - DMG installer, drag to Applications |
| `CodexSwitch-v3.20.0-macOS.zip`    | Unzip and drag to Applications, Universal Binary      |
| `CodexSwitch-v3.20.0-macOS.tar.gz` | For Homebrew install and auto-update                  |
...
- `CodexSwitch-v3.20.0-Linux-x86_64.AppImage` / `.deb` / `.rpm`
- `CodexSwitch-v3.20.0-Linux-arm64.AppImage` / `.deb` / `.rpm`

| Distribution                            | Recommended Format | Install Command                                                        |
| --------------------------------------- | ------------------ | ---------------------------------------------------------------------- |
| Ubuntu / Debian / Linux Mint / Pop!\_OS | `.deb`             | `sudo dpkg -i CodexSwitch-*.deb` or `sudo apt install ./CodexSwitch-*.deb` |
| Fedora / RHEL / CentOS / Rocky Linux    | `.rpm`             | `sudo rpm -i CodexSwitch-*.rpm` or `sudo dnf install ./CodexSwitch-*.rpm`  |
| openSUSE                                | `.rpm`             | `sudo zypper install ./CodexSwitch-*.rpm`                                |
| Arch Linux / Manjaro                    | `.AppImage`        | Make executable and run directly, or use AUR                           |
| Other distributions / unsure            | `.AppImage`        | `chmod +x CodexSwitch-*.AppImage && ./CodexSwitch-*.AppImage`              |
