...
Codex Switch v3.8.0 is a major architectural upgrade that restructures the data persistence layer and user interface, laying the foundation for future cloud sync and local proxy features.
...
- **Windows**: `CodexSwitch-v3.8.0-Windows.msi` or `-Portable.zip`
- **macOS**: `CodexSwitch-v3.8.0-macOS.tar.gz` or `.zip`
- **Linux**: `CodexSwitch-v3.8.0-Linux.AppImage` or `.deb`
