...
Codex Switch v3.8.0 はデータ永続化レイヤーと UI を大幅に作り替え、今後のクラウド同期やローカルプロキシ機能に向けた基盤を整えたメジャーアップデートです。
...
- **Windows**: `CodexSwitch-v3.8.0-Windows.msi` または `-Portable.zip`
- **macOS**: `CodexSwitch-v3.8.0-macOS.tar.gz` または `.zip`
- **Linux**: `CodexSwitch-v3.8.0-Linux.AppImage` または `.deb`
