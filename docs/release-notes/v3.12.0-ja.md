...
Codex Switch v3.12.0 は、プロバイダー互換性、OpenClaw の設定編集、共通設定の使い勝手、同期とデータ保守性を強化する機能リリースです。安定性を強化した **Model Health Check (Stream Check)** UI を復元し、**OpenAI Responses API** 形式変換を追加、**Ucloud**、**Micu**、**X-Code API**、**Novita**、**Bailian For Coding** などのプリセットを拡張し、**WebDAV 同期** に二層バージョニングを導入しました。
...
| ファイル                                 | 説明                                                 |
| ---------------------------------------- | ---------------------------------------------------- |
| `CodexSwitch-v3.12.0-Windows.msi`          | **推奨** - MSI インストーラー、自動更新対応          |
| `CodexSwitch-v3.12.0-Windows-Portable.zip` | ポータブル版、解凍して実行、レジストリ書き込みなし   |

### macOS

| ファイル                         | 説明                                                              |
| -------------------------------- | ----------------------------------------------------------------- |
| `CodexSwitch-v3.12.0-macOS.zip`    | **推奨** - 解凍して Applications にドラッグ、Universal Binary     |
| `CodexSwitch-v3.12.0-macOS.tar.gz` | Homebrew インストールと自動更新用                                 |
