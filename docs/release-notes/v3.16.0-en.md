...
>
> **Any "Codex Switch" website or client that asks you for payment, top-ups, or login credentials is fake.** If you have been tricked into paying, stop the transaction immediately and file a report through GitHub Issues so we can take down the imposter site as quickly as possible.

---

## Overview

Codex Switch v3.16.0's development since v3.15.0 centers on **promoting third-party Codex providers to first-class citizens through Chat Completions routing**. Codex natively only speaks the OpenAI Responses API and GPT-family models; this release lets Codex Switch's local proxy convert Codex's outgoing Responses requests into Chat Completions and rebuild the JSON and SSE streaming responses back into Responses shape, preserving `reasoning_content` / inline `<think>` blocks / streamed reasoning summaries / tool calls / `previous_response_id` follow-ups along the way, normalizing error envelopes, and probing Chat-format providers correctly in Stream Check. It ships 22 Chat-routing presets with explicit model catalogs (DeepSeek, Zhipu GLM, Kimi, MiniMax, StepFun, Baidu Qianfan, Bailian, ModelScope, Longcat, BaiLing, Xiaomi MiMo, Volcengine Agentplan, BytePlus, DouBao Seed, SiliconFlow, Novita AI, Nvidia, and more).
...
Installation simplified to `brew install --cask cc-switch` now that Codex Switch is in the official Homebrew repository; the personal-tap requirement was removed from all READMEs.
...
Codex loads `model_catalog_json` at startup, so after editing the model mapping table in Codex Switch you must **restart Codex** for the new catalog to take effect.

### Reasoning Effort May Have No Effect for Chat-Routing Providers

For providers that only expose a thinking on/off switch (Kimi, GLM, Qwen, MiniMax, MiMo, SiliconFlow), changing the reasoning effort in Codex (`model_reasoning_effort`: low / medium / high) **has no effect** — Codex Switch will not forward an unsupported effort field to them. Only providers with real effort tiers (DeepSeek, OpenRouter, and StepFun's `step-3.5-flash-2603` only) actually honor the level.
...
**Codex Third-Party Provider Chat Routing**: Converting and forwarding Codex requests through Codex Switch's local proxy to a third-party provider exposes those requests to that provider's billing, compliance, and data-retention policies — read the target provider's terms of service before using.

**Claude Desktop Third-Party Provider Switching via Proxy Gateway**: Routing Claude Desktop traffic through Codex Switch's in-app proxy gateway to a third-party provider exposes those requests to that provider's billing, compliance, and data-retention policies — read the target provider's terms of service before using.

By enabling these features, users **accept all associated risks**. Codex Switch is not responsible for any account restrictions, warnings, or service suspensions that result from using these features.
...
| File                                     | Description                                           |
| ---------------------------------------- | ----------------------------------------------------- |
| `CodexSwitch-v3.16.0-Windows.msi`          | **Recommended** - MSI installer, supports auto-update |
| `CodexSwitch-v3.16.0-Windows-Portable.zip` | Portable, extract and run, no registry writes         |

### macOS

| File                             | Description                                             |
| -------------------------------- | ------------------------------------------------------- |
| `CodexSwitch-v3.16.0-macOS.dmg`    | **Recommended** - DMG installer, drag into Applications |
| `CodexSwitch-v3.16.0-macOS.zip`    | Extract and drag into Applications, Universal Binary    |
| `CodexSwitch-v3.16.0-macOS.tar.gz` | For Homebrew installation and auto-update               |

> macOS builds are Apple code-signed and notarized — install directly.

### Homebrew (macOS)

> 🎉 Codex Switch is now available in the official Homebrew cask repository — no need to add a custom tap!
...
>
> - `CodexSwitch-v3.16.0-Linux-x86_64.AppImage` / `.deb` / `.rpm`
> - `CodexSwitch-v3.16.0-Linux-arm64.AppImage` / `.deb` / `.rpm`
...
| Other distros / not sure                | `.AppImage` | `chmod +x CodexSwitch-*.AppImage && ./CodexSwitch-*.AppImage`              |
