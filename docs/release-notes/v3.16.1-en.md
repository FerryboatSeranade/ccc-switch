# Codex Switch v3.16.1

> Codex stability patch: because some users did not want Codex Switch to change how Codex config files are written, Codex App Enhancements now has a switch and is off by default. After enabling it, you can keep using Codex mobile remote control, official plugins, and other official-app features while using third-party APIs; this release also includes a series of stability fixes.
...
Codex Switch v3.16.1 is a Codex stability patch following v3.16.0. v3.16.0 promoted third-party Codex providers to first-class citizens through Chat Completions routing; this release focuses on several high-risk edges discovered in real use: official ChatGPT / Codex OAuth login state could be overwritten while switching third-party providers or during local routing takeover, the Codex model catalog could be cleared during live backfill, hot switching, takeover shutdown restore, or editing the active provider, and Codex `tool_search`, plugin / connector namespace tools, and custom tools were not fully restored back into Responses events on the Chat Completions upstream path.

This release also hardens local routing takeover ownership checks. Provider switching and takeover toggles now run serially per app. When deciding whether the live files are proxy-managed, Codex Switch no longer relies only on stale `enabled` state or whether the proxy service is currently running; it also checks backups and proxy placeholders in the live files. This prevents ordinary live writes from overwriting proxy-managed config immediately after takeover is enabled, while the proxy is temporarily stopped, or during hot switching.
...
Added an optional setting for preserving official ChatGPT / Codex OAuth login state when switching to third-party Codex providers. When enabled, Codex Switch stores third-party provider API keys in the provider-scoped `experimental_bearer_token` inside Codex `config.toml` instead of overwriting the official login cache in `auth.json`.
...
When Codex is under local routing takeover, live `auth.json` / `config.toml` are temporarily rewritten by the proxy. Editing the active provider from those live files could incorrectly show proxy placeholders or official OAuth login as provider config. The editor now explicitly explains that it is showing the provider config stored in the database, not the proxy-managed live files; even if the proxy service is temporarily stopped, Codex Switch still treats the app as under takeover when the takeover state indicates so.
...
When Codex forwarding fails, Codex Switch now returns JSON errors that include provider, model, endpoint, upstream HTTP status, stable `cc_switch_*` error codes, and normalized HTTP status. This makes it much clearer which provider, endpoint, and upstream error caused the failure.
...
When local routing takeover is enabled, live `auth.json` / `config.toml` temporarily point to the Codex Switch proxy. The provider editor therefore shows the provider config saved in the database. This is expected; after takeover is disabled, Codex Switch restores live config from backups or the database source of truth.
...
| File                                     | Description                                     |
| ---------------------------------------- | ----------------------------------------------- |
| `CodexSwitch-v3.16.1-Windows.msi`          | **Recommended** - MSI installer with auto-update |
| `CodexSwitch-v3.16.1-Windows-Portable.zip` | Portable build, unzip and run                   |

### macOS

| File                             | Description                                           |
| -------------------------------- | ----------------------------------------------------- |
| `CodexSwitch-v3.16.1-macOS.dmg`    | **Recommended** - DMG installer, drag to Applications |
| `CodexSwitch-v3.16.1-macOS.zip`    | Unzip and drag to Applications, Universal Binary      |
| `CodexSwitch-v3.16.1-macOS.tar.gz` | For Homebrew install and auto-update                  |
...
- `CodexSwitch-v3.16.1-Linux-x86_64.AppImage` / `.deb` / `.rpm`
- `CodexSwitch-v3.16.1-Linux-arm64.AppImage` / `.deb` / `.rpm`
