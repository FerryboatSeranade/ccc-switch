...
**[Codex Switch で Claude Desktop プロバイダーを一括設定・管理・切り替える](../user-manual/ja/2-providers/2.6-claude-desktop.md)**
...
Codex Switch は初めて **Claude Desktop** を一等管理対象パネルとして扱い、Claude Code / Codex / Gemini / OpenCode / OpenClaw / Hermes と並列に位置づけます。

- Claude Desktop 専用パネルを追加し、Codex Switch 内蔵プロキシゲートウェイを介してサードパーティプロバイダーを Claude Desktop に代理転送
...
ChatGPT Codex 系プロバイダーはハードコードされたモデル選択に依存しなくなり、Codex Switch が必要に応じて ChatGPT バックエンドから**ライブモデルリスト**を取得します。
...
Claude Desktop のプロバイダーを切り替えると Codex Switch 管理の 3P プロファイルが書き込まれます。**Claude Desktop の再起動**が必要です; プロキシモードのプロバイダーは、使用中 Codex Switch の Local Routing が動作し続けている必要があります。
...
### Codex Switch ブランド公式サイトを trusted release channel に統一
...
これら 20 個のプリセットは以前はデフォルトでプロキシ経由でルーティングされていましたが、現在はデフォルトで直接接続です。アップグレード前にこのうちのいずれかを使用しており、かつプロキシルーティングの接続性に依存していた場合（例: プロキシに特殊な補正器や変換層がある場合）、接続性を検証してください; 必要に応じて、Codex Switch パネル内で手動でプロキシモードに戻すことができます。

### Claude Desktop の操作制約

Claude Desktop プロバイダーの切り替えには、**Claude Desktop の再起動**が必要です; プロキシモードのプロバイダーは、使用中 Codex Switch の Local Routing が動作し続けている必要があります — Codex Switch を終了させたり Local Routing を停止させたりすると、プロキシモードの Claude Desktop プロバイダーへの接続が切断されます。
...
| ファイル                                 | 説明                                        |
| ---------------------------------------- | ------------------------------------------- |
| `CodexSwitch-v3.15.0-Windows.msi`          | **推奨** - MSI インストーラー、自動更新対応 |
| `CodexSwitch-v3.15.0-Windows-Portable.zip` | ポータブル版、解凍して実行、レジストリ不要  |

### macOS

| ファイル                         | 説明                                                   |
| -------------------------------- | ------------------------------------------------------ |
| `CodexSwitch-v3.15.0-macOS.dmg`    | **推奨** - DMG インストーラー、Applications にドラッグ |
| `CodexSwitch-v3.15.0-macOS.zip`    | 解凍して Applications にドラッグ、Universal Binary     |
| `CodexSwitch-v3.15.0-macOS.tar.gz` | Homebrew インストールと自動更新用                      |
