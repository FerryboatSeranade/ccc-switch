...
The local proxy now responds to `GET /v1/models`, which Codex CLI probes at startup, returning the Codex Switch-managed Codex model catalog. A stale-catalog guard was added: it parses the live `config.toml` and only serves the catalog when `model_catalog_json` still points at the Codex Switch-owned catalog file, avoiding exposing a previous provider's leftover catalog to Codex (#3818).
...
When a Chat Completions upstream ends a stream without a `finish_reason` or `[DONE]`, Codex Switch no longer treats it as a normal completion: it finalizes normally only when the stream truly ended; emits an incomplete (`max_output_tokens`) response when partial output was produced; and emits a failed `stream_truncated` event when nothing was produced. Late-arriving reasoning is also backfilled onto still-active streaming tool calls.
...
On Windows, quitting Codex Switch could leave a dead tray icon behind until the mouse passed over it. The app now explicitly removes the tray icon before exiting, so it disappears cleanly when the process ends (#3797).

### Windows Taskbar Icon

Sets an explicit Windows AppUserModelID at runtime and writes the same ID and product icon onto the installer's desktop and start-menu shortcuts, so Codex Switch shows the correct icon and groups properly in the taskbar (#3457).
...
| File                                     | Description                                      |
| ---------------------------------------- | ------------------------------------------------ |
| `CodexSwitch-v3.16.2-Windows.msi`          | **Recommended** - MSI installer with auto-update |
| `CodexSwitch-v3.16.2-Windows-Portable.zip` | Portable build, unzip and run                    |

### macOS

| File                             | Description                                           |
| -------------------------------- | ----------------------------------------------------- |
| `CodexSwitch-v3.16.2-macOS.dmg`    | **Recommended** - DMG installer, drag to Applications |
| `CodexSwitch-v3.16.2-macOS.zip`    | Unzip and drag to Applications, Universal Binary      |
| `CodexSwitch-v3.16.2-macOS.tar.gz` | For Homebrew install and auto-update                  |
...
- `CodexSwitch-v3.16.2-Linux-x86_64.AppImage` / `.deb` / `.rpm`
- `CodexSwitch-v3.16.2-Linux-arm64.AppImage` / `.deb` / `.rpm`
