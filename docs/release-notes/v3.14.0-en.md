...
- **Application-Level Window Controls**: Opt-in setting to render Codex Switch's own min/max/close buttons, materially improving Linux Wayland experience
- **Hermes in Unified Skills Management**: Skill install, enable, and filter now cover Hermes
- **Hermes / OpenClaw Config Directory Override**: Point Codex Switch at a custom `~/.hermes/config.yaml` or `openclaw.json` location
...
Codex Switch now treats Hermes Agent as a first-class managed app alongside Claude / Codex / Gemini / OpenCode / OpenClaw.
...
- **User `providers:` Dict Read-Only Rendering**: User-authored providers in the YAML appear as read-only cards in Codex Switch; deep configuration delegates to the Hermes Web UI
...
- Opt-in setting to render Codex Switch's own minimize / toggle-maximize / close buttons instead of system decorations (#1119, thanks @git1677967754)
...
- New settings option to point Codex Switch at a custom `openclaw.json` location (#1518, thanks @mrFranklin)

### Hermes Config Directory Override

- New settings option to point Codex Switch at a custom `~/.hermes/config.yaml` location, backed by data-driven dispatch
...
- Deep YAML knobs are no longer duplicated in the Codex Switch form — they are delegated to the Hermes Web UI via a direct launch action
...
- Round-tripping through Codex Switch no longer drops OAuth MCP `auth` blocks or pollutes unrelated YAML keys
...
| File                                     | Description                                     |
| ---------------------------------------- | ----------------------------------------------- |
| `CodexSwitch-v3.14.0-Windows.msi`          | **Recommended** - MSI installer, supports auto-update |
| `CodexSwitch-v3.14.0-Windows-Portable.zip` | Portable, extract and run, no registry writes   |

### macOS

| File                             | Description                                              |
| -------------------------------- | -------------------------------------------------------- |
| `CodexSwitch-v3.14.0-macOS.dmg`    | **Recommended** - DMG installer, drag into Applications  |
| `CodexSwitch-v3.14.0-macOS.zip`    | Extract and drag into Applications, Universal Binary     |
| `CodexSwitch-v3.14.0-macOS.tar.gz` | For Homebrew installation and auto-update                |
