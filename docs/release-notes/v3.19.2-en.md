...
- **Search directly inside the MCP, prompt, and Skills panels**: all three panels now have a search box; the per-app badges on the MCP and Skills lists also become three-state toggles that enable or disable an app across the whole list in one click ([#5954](https://github.com/FerryboatSeranade/ccc-switch/issues/5954) and [#5935](https://github.com/FerryboatSeranade/ccc-switch/issues/5935) both came straight from user requests).
...
Codex Switch v3.19.2 is a correctness-focused maintenance release along three lines. The first is getting the numbers right: the Codex session importer could over-count usage several times over on files with interleaved counters ([#3011](https://github.com/FerryboatSeranade/ccc-switch/issues/3011)); this release switches to an algorithm that prefers each turn's explicit delta and recognizes replays by their full signature, closed out by replaying nearly 1,900 real session files; the backup and usage-import write paths are batched at the same time, markedly easing import stalls on large databases ([#2100](https://github.com/FerryboatSeranade/ccc-switch/issues/2100) will likely benefit).
...
The MCP, prompt, and Skills panels share a new search box — Esc clears it, and it only intercepts the global back shortcut while it has content, so your usual habits are unaffected. The per-app count badges atop the MCP and Skills lists become three-state toggle buttons: one click enables or disables an app across the whole list — the days of flipping switches row by row are over ([#5954](https://github.com/FerryboatSeranade/ccc-switch/issues/5954), [#5935](https://github.com/FerryboatSeranade/ccc-switch/issues/5935)).
...
Now, when every tool call in a turn has been dropped and none remains usable, the streaming path emits `response.failed` and the non-streaming path reports a transform error; the check is gated on `status == "completed"`, so `finish_reason: length` truncation keeps its own `incomplete` semantics. All three drop sites log structured, content-free fields — call-id presence, argument byte counts, finish reason — so problems like [#4341](https://github.com/FerryboatSeranade/ccc-switch/issues/4341) can finally be diagnosed from real traffic. Turns with a valid tool call, text-only turns, and truncated turns behave as before.

### OMO's Configuration Was Written to a File It No Longer Reads

OMO 4.19.3 unified its configuration into `~/.omo/omo.jsonc` (then `omo.json`), and its migration renames the legacy per-app file out of the way. From then on Codex Switch — which only knew the legacy path — kept writing a file outside OMO's config chain, so provider switches looked successful while landing nowhere ([#5945](https://github.com/FerryboatSeranade/ccc-switch/issues/5945)).
...
The route status indicator's pulse animation kept running while the window was unfocused, occupying the GPU for a purely decorative effect. Window focus now gates the heartbeat through a data attribute and CSS — an unfocused window freezes the animation at full opacity, and the system's reduced-motion preference disables it entirely. Data polling is unaffected; only the decoration pauses. ([#5767](https://github.com/FerryboatSeranade/ccc-switch/pull/5767))
...
A six-part externally contributed hardening pass ([#5919](https://github.com/FerryboatSeranade/ccc-switch/pull/5919)):
...
Old single-row backups still import as before, and the new format is within what every SQLite shipped with the app supports — compatible in both directions across versions. The cross-machine import freeze reported in [#2100](https://github.com/FerryboatSeranade/ccc-switch/issues/2100) will likely ease — real-world results on that issue are welcome. ([#6122](https://github.com/FerryboatSeranade/ccc-switch/pull/6122))
...
- [#5854](https://github.com/FerryboatSeranade/ccc-switch/pull/5854): the Codex interleaved-counter usage fix — this release's headline. Thanks to @MJYKIM99, who arrived with real-file evidence and polished the fix through three review rounds until the full replay converged; thanks also to @ayanamislover for independently replaying their own interleaved files, corroborating our validation.
- [#5919](https://github.com/FerryboatSeranade/ccc-switch/pull/5919), [#6122](https://github.com/FerryboatSeranade/ccc-switch/pull/6122), [#6119](https://github.com/FerryboatSeranade/ccc-switch/pull/6119): the six-part security hardening, the backup performance overhaul, and the skills.sh README fix. Thanks to @zayokami — the "Security Hardening" and "Performance" chapters of this release are largely these three pieces of work.
- [#6011](https://github.com/FerryboatSeranade/ccc-switch/pull/6011), [#5522](https://github.com/FerryboatSeranade/ccc-switch/pull/5522), [#5767](https://github.com/FerryboatSeranade/ccc-switch/pull/5767): the OMO unified-config adaptation, runtime model discovery, and the route-status GPU fix. Thanks to @allenxu09.
- [#5967](https://github.com/FerryboatSeranade/ccc-switch/pull/5967): the management panels' search and bulk toggles, fixing two long-standing data-layer concurrency problems along the way. Thanks to @YUZHEthefool — who also collaborated on #6119 to converge that fix to its minimal shape.
- [#4887](https://github.com/FerryboatSeranade/ccc-switch/pull/4887): per-account subscription usage in the Auth Center. Thanks to @SaladDay.
- [#5832](https://github.com/FerryboatSeranade/ccc-switch/pull/5832): Copilot takeover compatibility with current Claude Code. Thanks to @Suaig.
- [#5779](https://github.com/FerryboatSeranade/ccc-switch/pull/5779): the Hermes prompt file-name fix. Thanks to @mmm-05610 — from the report [#5777](https://github.com/FerryboatSeranade/ccc-switch/issues/5777) to the submitted fix, one person end to end.
- [#4153](https://github.com/FerryboatSeranade/ccc-switch/pull/4153): anchoring Skill source-directory resolution on SKILL.md. Thanks to @makoMakoGo.
- [#6053](https://github.com/FerryboatSeranade/ccc-switch/pull/6053): built-in pricing for Qwen3.8 Max. Thanks to @mhy1227.

### Issue Reports

- Thanks to @KeShih for pinpointing in [#3011](https://github.com/FerryboatSeranade/ccc-switch/issues/3011) that `total_token_usage` is non-monotonic and that replays were being counted as new increments — the problem definition behind this release's headline fix comes straight from that report; @KeShih also proposed the repair direction early in [#3015](https://github.com/FerryboatSeranade/ccc-switch/issues/3015).
- Thanks to @Syuryuuki for reporting the missing OMO unified-config adaptation ([#5945](https://github.com/FerryboatSeranade/ccc-switch/issues/5945)), @abcfy2 for the ast-grep Skill install failure ([#4141](https://github.com/FerryboatSeranade/ccc-switch/issues/4141)), and @mortalBibo for the skills.sh nested-Skill 404 links ([#6111](https://github.com/FerryboatSeranade/ccc-switch/issues/6111)).
- Thanks to @kith13 ([#5954](https://github.com/FerryboatSeranade/ccc-switch/issues/5954)) and @Getianyu1998 ([#5935](https://github.com/FerryboatSeranade/ccc-switch/issues/5935)) — the management panels' search and bulk toggles came straight from these two suggestions.
...
| File                                     | Description                                      |
| ---------------------------------------- | ------------------------------------------------ |
| `CodexSwitch-v3.19.2-Windows.msi`          | **Recommended** - MSI installer with auto-update |
| `CodexSwitch-v3.19.2-Windows-Portable.zip` | Portable build, unzip and run                    |
...
| File                             | Description                                           |
| -------------------------------- | ----------------------------------------------------- |
| `CodexSwitch-v3.19.2-macOS.dmg`    | **Recommended** - DMG installer, drag to Applications |
| `CodexSwitch-v3.19.2-macOS.zip`    | Unzip and drag to Applications, Universal Binary      |
| `CodexSwitch-v3.19.2-macOS.tar.gz` | For Homebrew install and auto-update                  |
...
- `CodexSwitch-v3.19.2-Linux-x86_64.AppImage` / `.deb` / `.rpm`
- `CodexSwitch-v3.19.2-Linux-arm64.AppImage` / `.deb` / `.rpm`
