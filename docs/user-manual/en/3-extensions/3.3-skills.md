...
CCodex Switch comes pre-configured with the following GitHub repositories:
...
CCodex Switch provides powerful search and filter features:
...
CCodex Switch compares installed skills with the remote repository version using **SHA-256 content hashes**. Whenever the remote has any content changes, the corresponding local skill card automatically shows an "Update available" indicator.
...
2. CCodex Switch batch-downloads all skills with pending updates
...
| ------------------------ | --------------------------------------------------------------------- |
| **CCodex Switch built-in**   | Default location `~/.cc-switch/skills/`, managed by CCodex Switch         |
| **`~/.agents/skills`**   | A shared directory conforming to community agent tool conventions, better for cross-tool collaboration |

### How to Switch

Select the target storage location from the settings or management menu in the Skills panel. The switch **does not lose skill state** — CCodex Switch smoothly migrates existing skills to the new location.
...
v3.13.0 integrates **skills.sh** public registry search so you can discover community skills directly inside CCodex Switch.
...
- CCodex Switch has not finished the latest scan
