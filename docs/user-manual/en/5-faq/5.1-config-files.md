...
Hermes uses YAML configuration. CCodex Switch writes MCP servers to `mcp_servers`, writes editable provider entries to `custom_providers`, reads read-only entries from Hermes' `providers` dict, and updates `model.provider` / `model.default` when switching providers.
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to CCodex Switch's "providers") |
...
CCodex Switch's priority when modifying configurations:

1. **CCodex Switch Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by CCodex Switch)
- CCodex Switch's `settings.json`
...
CCodex Switch v3.7.0 migrated from JSON files to SQLite:
