...
CCodex Switch automatically detects conflicts between system environment variables and app configurations, preventing configurations from being unexpectedly overridden.
...
Found X environment variables that may conflict with CCodex Switch configuration
[Expand] [Dismiss]
```
...
3. CCodex Switch will automatically back up and remove the selected variables
...
If you prefer not to use CCodex Switch to remove variables, you can handle them manually:
...
- CCodex Switch provider configurations being overridden
- API requests being sent to the wrong endpoint
- Using the wrong API key

## Best Practices

1. **Use CCodex Switch to manage configurations**: Avoid setting API keys in system environment variables
