...
CCodex Switch for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the Releases page.
...
2. Add CCodex Switch to your antivirus software's whitelist
...
chmod +x CCodexSwitch-*.AppImage

# If it still fails, try
./CCodexSwitch-*.AppImage --no-sandbox
```
...
**Solutions**:
1. Confirm the file was exported by CCodex Switch
...
**Windows**:
- Check taskbar settings to ensure the CCodex Switch icon is not hidden
...
Toggle "Lightweight Mode" from the system tray menu. The main window closes, and CCodex Switch runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, CCodex Switch covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `cccccswitch://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
4. Provide the following information:
   ...
   - CCodex Switch version
   - Problem description and reproduction steps
   - Error messages (if any)

### Log Files

Codex Switch stores logs under its application configuration directory. By default this is `.cc-switch` in your home directory; if you changed the configuration directory in Advanced Settings, use that custom location instead.
