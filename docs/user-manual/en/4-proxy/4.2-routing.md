...
App routing means letting CCodex Switch route a specific application's API requests through the local routing service.
...
When routing is enabled, CCodex Switch modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, CCodex Switch will:
...
Before enabling routing, CCodex Switch backs up the original configuration and restores it when disabled.
