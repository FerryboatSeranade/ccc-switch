...
- This release migrates the database schema from v16 to v17 (a new session-usage dedup ledger for the Pi importer); a pre-migration backup is created automatically. After running this version once, older Codex Switch builds refuse to open the database — downgrading requires restoring that backup.
...
- If an earlier version already rewrote `model_catalog_json` in `~/.codex/config.toml`, this release does not restore your original pointer — point it back at your own file once, and it will be left alone from then on. While it points at a user-owned file, Codex Switch's per-provider model table (display names, context windows, reasoning levels) does not reach Codex.
...
- **Hermes Prompts Were Written to a File Hermes Never Reads**: Hermes loads its persona file `SOUL.md` from `~/.hermes/` and never looks for `AGENTS.md` there — that name is project-level context discovered from the working directory upward. Codex Switch's prompt management wrote `~/.hermes/AGENTS.md` from the day Hermes support was added, so enabling a Hermes prompt produced a dead file. Prompts now read and write `~/.hermes/SOUL.md`, and the existing back-fill still applies: a SOUL.md you wrote yourself is imported into the database before being replaced. (#5777)
...
- **Grok Build Joins the Failover Tabs and the Environment-Conflict Check**: The failover settings gain a fourth tab for Grok Build alongside Claude Code, Codex and Gemini, and the startup environment-conflict banner now detects a shell-exported `XAI_API_KEY` or `GROK_DEFAULT_MODEL` — variables that silently override whatever provider is selected. Detection distinguishes exact names from prefixes, so Codex Switch's own `GROK_BIN_DIR` and `GROK_HOME` are not reported.
...
- The mirrored DeepSeek catalog declares a minimum Codex client version of 0.144.0, which Codex Switch does not verify — the freeform `apply_patch` registration it carries needs that release or newer. The generated catalog file also grows to roughly 75 KB for the two mirrored models, since each carries the full harness text.
...
- **In-App Updates Use a Release Mirror**: The updater now queries the configured release mirror for the latest manifest before falling back to GitHub Releases, so checking for and downloading updates no longer depends on GitHub being reachable, which is slow or blocked for many users. The mirrored manifest points every platform's download at the same bucket while minisign signatures are left untouched: they cover file contents rather than URLs, so each downloaded artifact is still verified against the public key baked into the app and the mirror itself remains untrusted. Publishing is handled by a release-gated sync workflow that only rewrites the root manifest when its tag really is GitHub's `releases/latest`, so the mirror can never push users backwards to an older version.
...
- **Release Assets Mirrored to Cloudflare R2 for Downloads**: Release automation mirrors every installer to an R2 bucket and generates the manifest consumed by the download page — per-file SHA-256 checksums, sizes, and the release's real publish date — with immutable caching for versioned assets, a five-minute TTL on the root manifest, and pruning beyond the five most recent versions. Forks without the R2 credentials skip the sync entirely.
...
- The updater endpoint list ships inside the app binary, so existing installations keep checking GitHub until they have updated once to a build containing this change; from then on the release mirror is preferred with GitHub as fallback.
...
- **Codex Chat Truncated Stream Detection**: When a Chat Completions upstream ends a stream without a `finish_reason` or `[DONE]`, Codex Switch no longer reports it as a normal completion — it finalizes normally only when the stream truly finished, emits an incomplete (`max_output_tokens`) response when partial output was produced, and emits a failed `stream_truncated` event when nothing was produced. Late-arriving reasoning is also attached to still-active streamed tool calls.
...
- **Windows Tray Icon Residue on Exit**: Quitting Codex Switch on Windows could leave a dead tray icon until hovered; the app now removes the tray icon before exiting so it disappears cleanly (#3797).
- **Windows Taskbar Icon**: Set an explicit Windows AppUserModelID at runtime and stamped the installer's desktop and start-menu shortcuts with the same ID and product icon, so Codex Switch shows the correct icon and groups properly in the taskbar (#3457).
...
- **Codex Chat Completions routing**: Codex providers can now be served by OpenAI-compatible Chat Completions upstreams. Codex Switch converts Codex Responses requests into Chat Completions, rebuilds JSON and SSE responses back into Responses shape, preserves reasoning / `<think>` / tool-call state, normalizes error envelopes, and probes Chat-format providers correctly in Stream Check.
...
- **Codex Chat Completions Routing**: Codex providers can now be served by upstreams that only speak the OpenAI Chat Completions API. Codex Switch's local proxy converts Codex's outgoing Responses requests into Chat Completions and rebuilds the Chat response (both JSON and SSE) back into Responses shape, preserving `reasoning_content`, inline `<think>` blocks, streamed reasoning summaries, tool calls, and `previous_response_id` follow-ups. A bounded Codex Chat history cache restores tool calls before their tool outputs.
...
- **Homebrew Official Cask Installation**: Installation simplified to `brew install --cask cc-switch` now that Codex Switch is in the official Homebrew repository; the personal-tap requirement was removed from all READMEs.
...
- **Claude Desktop Third-Party Provider Switching via Proxy Gateway**: Added a dedicated Claude Desktop surface that brokers third-party Claude providers through Codex Switch's in-app proxy, with a routing-support badge for providers that need it, role-based model route mapping locked to `sonnet` / `opus` / `haiku`, Copilot/Codex OAuth provider reuse, a redesigned Claude Code import flow, app-switcher differentiation between "Claude Code" and "Claude Desktop", and 44 provider presets translated from the Claude Code catalog.
...
- **Claude Desktop Operational Notes**: Switching a Claude Desktop provider now writes Codex Switch's managed 3P profile and requires restarting Claude Desktop to take effect; proxy-mode providers require Codex Switch Local Routing to stay running.
...
- **Codex Switch Brand Surface Unified**: All in-app and README references now point at the project's official release pages; the release notes template uses the same project identity.
...
- **Release Notes Template**: Uses the project's official release pages.
- **Brand Surface**: Documents the project's official repository and release pages across READMEs and in-app references.
...
- **Hermes Config Health Scanner**: Removed the in-app Hermes config health scanner, warning banner, `scan_hermes_config_health` command, `HermesHealthWarning` type, and `HermesWriteOutcome.warnings` payload. Codex Switch now keeps the Hermes surface focused on active provider display, provider switching defaults, memory editing, and launching the Hermes Web UI for deep configuration.
...
- **Codex OAuth Responses SSE Aggregation**: Non-streaming Anthropic clients now receive JSON even when the ChatGPT Codex upstream forces OpenAI Responses SSE; Codex Switch aggregates the upstream SSE events before running the non-streaming transform (#2235).
...
- **Hermes Memory Panel**: Added a Memory panel for editing `MEMORY.md` and `USER.md` directly from Codex Switch, with an enable switch, character-count limits, and a live save flow. Replaces the Prompts entry for Hermes.
...
- **Application-Level Window Controls**: Added an opt-in setting to render Codex Switch's own minimize / toggle-maximize / close buttons instead of the system decorations, materially improving the experience on Linux Wayland where compositor-drawn buttons can become inert.
...
- **OpenClaw Config Directory Override**: Added a settings option to point Codex Switch at a custom `openclaw.json` location.
- **Hermes Config Directory Override**: Added a settings option to point Codex Switch at a custom `~/.hermes/config.yaml` location, backed by data-driven dispatch.
...
- **Hermes Deep Config Delegation**: Deep YAML knobs are now delegated to the Hermes Web UI via a direct launch action, rather than duplicated in the Codex Switch form.
...
- **Hermes YAML Pollution & OAuth MCP Auth Drop**: Round-tripping through Codex Switch no longer drops OAuth MCP `auth` blocks or pollutes unrelated YAML keys; guard tests added via `tests/hermes_roundtrip.rs`.
...
- **Lightweight Mode**: Added a tray-only mode that destroys the main window and keeps Codex Switch running from the system tray, with the window recreated when users reopen it.
...
- **Skills Discovery & Batch Updates**: Added SHA-256 based skill update detection, per-skill and batch update actions, a storage-location toggle between Codex Switch and `~/.agents/skills`, and public `skills.sh` search integration.
...
- **OpenClaw Integration**: Full management support for OpenClaw as the fifth application in Codex Switch, including provider switching, configuration panels (Env / Tools / Agents Defaults), Workspace file management (HEARTBEAT / BOOTSTRAP / BOOT), daily memory files, and additive overlay mode
...
- **Windows Title** - Display "Codex Switch" instead of default "Tauri app" in window title
...
- **Import/Export** - Restrict SQL import to Codex Switch exported backups only; refresh providers immediately after import
...
- **Deep link support** - Import Gemini providers via `cccswitch://` protocol
...
- **Protocol registration** - `cccswitch://` URL scheme for one-click imports
...
- **Standardized release artifact naming** - All platform releases now use consistent version-tagged filenames:
  - macOS: `CodexSwitch-v{version}-macOS.tar.gz` / `.zip`
  - Windows: `CodexSwitch-v{version}-Windows.msi` / `-Portable.zip`
  - Linux: `CodexSwitch-v{version}-Linux.AppImage` / `.deb`
